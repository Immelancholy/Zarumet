...
pub fn render_years_mode(
    ...
    if let Some(library) = library {
        ...
        let year_list: Vec<ratatui::widgets::ListItem> = library
            ...
            .iter()
            .map(|year| {
                // Calculate available width for year (subtract borders and padding)
                ...
                let truncated_name = WIDTH_CACHE.with(|cache| {
                    ...
                    utils::truncate_by_width_cached(&mut cache, &year.0, available_width)
                });
                ratatui::widgets::ListItem::new(vec![Line::from(truncated_name)])
            })
            .collect();

        let year_border_style = if panel_focus == &PanelFocus::YearList {
            Style::default().fg(config.colors.border_selected_color())
        } else {
            Style::default().fg(config.colors.border_color())
        };

        let year_list_widget = ratatui::widgets::List::new(year_list)
            .block(
                Block::default()
                    .borders(Borders::ALL)
    ...
    // Show albums for selected year, or empty tracks box
    if let (Some(library), Some(selected_year_index)) = (library, year_list_state.selected()) {
        if let Some(selected_year) = library.albums_by_year.get(selected_year_index) {
            // Only initialize album selection if albums panel is focused
            if year_albums_list_state.selected().is_none()
                && panel_focus == &PanelFocus::YearAlbums
                && !selected_year.1.is_empty()
            {
                year_albums_list_state.select(Some(0));
            }
            ...
            let albums_border_color = if panel_focus == &PanelFocus::YearAlbums {
            ...
            let albums_highlight_style = if panel_focus == &PanelFocus::YearAlbums {
                Style::default()
                    .fg(config.colors.queue_selected_text_color())
                    .bg(config.colors.queue_selected_highlight_color())
            } else {
