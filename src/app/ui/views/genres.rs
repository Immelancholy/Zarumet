use crate::app::{
    ...
    ui::{
        DisplayItem, Protocol, RENDER_CACHE, WIDTH_CACHE, compute_albums_display_list_genres,
        rendering::utils,
        widgets::{
...
pub fn render_genres_mode(
    ...
    let left_horizontal_chunks = Layout::horizontal([
        Constraint::Percentage(35), // Genres box takes 35% of left space
    ...
    if let Some(library) = library {
        if genre_list_state.selected().is_none() && !library.albums_by_genre.is_empty() {
            genre_list_state.select(Some(0));
        }
        let genre_list: Vec<ratatui::widgets::ListItem> = library
            .albums_by_genre
            .iter()
            .map(|genre| {
                // Calculate available width for genre (subtract borders and padding)
                ...
                let truncated_name = WIDTH_CACHE.with(|cache| {
                    ...
                    utils::truncate_by_width_cached(&mut cache, &genre.0, available_width)
                });
        ...
        let genre_list_widget = ratatui::widgets::List::new(genre_list)
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
                    .title(Line::from(" Genre ").fg(config.colors.border_title_color()))
                    .border_style(genre_border_style),
            )
            .highlight_style(
                Style::default()
    ...
    // Show albums for selected genre, or empty tracks box
    if let (Some(library), Some(selected_genre_index)) = (library, genre_list_state.selected()) {
        if let Some(selected_genre) = library.albums_by_genre.get(selected_genre_index) {
            // Only initialize album selection if albums panel is focused
            if genre_albums_list_state.selected().is_none()
                && panel_focus == &PanelFocus::GenreAlbums
                && !selected_genre.1.is_empty()
            {
                genre_albums_list_state.select(Some(0));
            }
            ...
            let (display_items, _album_indices) =
                compute_albums_display_list_genres(&selected_genre.1, expanded_albums_genres);

            if genre_albums_display_list_state.selected().is_none() && !display_items.is_empty() {
                genre_albums_display_list_state.select(Some(0));
            }
                ...
                .map(|item| {
                    match item {
                        ...
                        DisplayItem::Song(song_title, duration, _file_path) => {
                            ...
                            let filler = RENDER_CACHE.with(|cache| {
                                cache.borrow().fillers.spaces(filler_width).to_owned()
                            });
            ...
            let albums_highlight_style = if panel_focus == &PanelFocus::GenreAlbums {
                Style::default()
                    .fg(config.colors.queue_selected_text_color())
                    .bg(config.colors.queue_selected_highlight_color())
            } else {
