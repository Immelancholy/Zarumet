use crate::app::{
    ...
    ui::{
        DisplayItem, Protocol, RENDER_CACHE, WIDTH_CACHE, compute_albums_display_list_uris,
        rendering::utils,
        widgets::{
...
pub fn render_uris_mode(
    ...
    let left_horizontal_chunks = Layout::horizontal([
        Constraint::Percentage(35), // uris box takes 35% of left space
        Constraint::Percentage(75), // Albums box takes 75% of left space
    ])
    ...
    if let Some(library) = library {
        if uri_list_state.selected().is_none() && !library.albums_by_uri.is_empty() {
            uri_list_state.select(Some(0));
        }
        let uri_list: Vec<ratatui::widgets::ListItem> = library
            .albums_by_uri
            .iter()
            .map(|uri| {
                // Calculate available width for uri (subtract borders and padding)
                ...
                let truncated_name = WIDTH_CACHE.with(|cache| {
                    ...
                    utils::truncate_by_width_cached(&mut cache, &uri.0, available_width)
                });
                ratatui::widgets::ListItem::new(vec![Line::from(truncated_name)])
            })
            .collect();

        let uri_border_style = if panel_focus == &PanelFocus::UriList {
            Style::default().fg(config.colors.border_selected_color())
        } else {
            Style::default().fg(config.colors.border_color())
        };

        let uri_list_widget = ratatui::widgets::List::new(uri_list)
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
                    .title(Line::from(" uri ").fg(config.colors.border_title_color()))
                    .border_style(uri_border_style),
            )
            .highlight_style(
                Style::default()
        ...
        let uris_box = create_empty_box("Uris", config);
        frame.render_widget(uris_box, left_horizontal_chunks[0]);
    }

    // Show albums for selected uri, or empty tracks box
    if let (Some(library), Some(selected_uri_index)) = (library, uri_list_state.selected()) {
        if let Some(selected_uri) = library.albums_by_uri.get(selected_uri_index) {
            // Only initialize album selection if albums panel is focused
            if uri_albums_list_state.selected().is_none()
                && panel_focus == &PanelFocus::UriAlbums
                && !selected_uri.1.is_empty()
            {
                uri_albums_list_state.select(Some(0));
            }
            ...
            let (display_items, _album_indices) =
                compute_albums_display_list_uris(&selected_uri.1, expanded_albums_uris);

            if uri_albums_display_list_state.selected().is_none() && !display_items.is_empty() {
                uri_albums_display_list_state.select(Some(0));
            }
                ...
                .map(|item| {
                    match item {
                        DisplayItem::Album(album_name) => {
                            ...
                            let duration_str =
                                RENDER_CACHE.with(|cache| match album.1.total_duration() {
                                    Some(duration) => {
                                        let mut cache = cache.borrow_mut();
            ...
            let albums_border_color = if panel_focus == &PanelFocus::UriAlbums {
            ...
            let albums_highlight_style = if panel_focus == &PanelFocus::UriAlbums {
                Style::default()
                    .fg(config.colors.queue_selected_text_color())
                    .bg(config.colors.queue_selected_highlight_color())
            } else {
